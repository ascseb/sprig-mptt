...
class UnitTest_Sprig_MPTT extends UnitTest_Case {
	...
	{
		$node_5_with_parent = Sprig::factory('mptt_test', array('id' => 5))->load(NULL, 1, array('parent'));
		$root = $node_5_with_parent->root();

		$this
			->assert_similar($node_5_with_parent->parent, $root)
			->assert_false($root->parent->loaded());
	}
	...
	function test_ancestors()
	{
		...
		$node_5_ancestors = $node_5->ancestors();
		$node_9_ancestors = $node_9->ancestors();
		$node_9->ancestors($query, TRUE, 'DESC');
		$node_9_ancestors_desc = Sprig::factory('mptt_test')->load($query, FALSE);

		$this
			->assert_instance($node_5_ancestors, 'Database_Result')
			->assert_count($node_5_ancestors, 1)
			->assert_count($node_5->ancestors($node_5_query, FALSE)->load($node_5_query, FALSE), 0)
			->assert_equal($node_5_ancestors[0]->id, 1)

			->assert_instance($node_9_ancestors, 'Database_Result')
			->assert_count($node_9_ancestors, 3)
			->assert_count($node_9->ancestors($node_9_query, FALSE)->load($node_9_query, FALSE), 2)
			->assert_equal($node_9_ancestors[0]->id, 1)
			->assert_equal($node_9_ancestors[1]->id, 6)
			->assert_equal($node_9_ancestors[2]->id, 8)

			->assert_equal($node_9_ancestors_desc[2]->id, 1)
			->assert_equal($node_9_ancestors_desc[1]->id, 6)
			->assert_equal($node_9_ancestors_desc[0]->id, 8);
	}

	function test_with_ancestors()
	{
		$node_5_with_ancestors = Sprig::factory('mptt_test', array('id' => 5))->load(NULL, FALSE, array('ancestors'));
		$node_9_with_ancestors = Sprig::factory('mptt_test', array('id' => 9))->load(NULL, FALSE, array('ancestors'));

		$this
			->assert_instance($node_5_with_ancestors, 'Database_Result')
			->assert_count($node_5_with_ancestors, 1)
			->assert_equal($node_5_with_ancestors[0]->ancestor->id, 1)

			->assert_instance($node_9_with_ancestors, 'Database_Result')
			->assert_count($node_9_with_ancestors, 3)
			->assert_equal($node_9_with_ancestors[0]->ancestor->id, 1)
			->assert_equal($node_9_with_ancestors[1]->ancestor->id, 6)
			->assert_equal($node_9_with_ancestors[2]->ancestor->id, 8);
	}
	...
	function test_children()
	{
		...
		$node_6->children($query, FALSE, 'DESC');
		$node_6_children_desc = Sprig::factory('mptt_test')->load($query, FALSE);

		$this
			->assert_instance($node_6_children, 'Database_Result')
			->assert_count($node_6_children, 3)
			->assert_count($node_6->children($node_6, TRUE)->load($node_6, FALSE), 4)
	...
	{
		$node_6_with_children = Sprig::factory('mptt_test', array('id' => 6))->load(NULL, FALSE, array('children'));

		$this
			->assert_instance($node_6_with_children, 'Database_Result')
			->assert_count($node_6_with_children, 3)
			->assert_equal($node_6_with_children[0]->child->id, 7)
			->assert_equal($node_6_with_children[1]->child->id, 8)
			->assert_equal($node_6_with_children[2]->child->id, 11);
	}
	...
	function test_descendants()
	{
		...
		$node_6->descendants($node_6_descendants);
		$node_6_descendants = Sprig::factory('mptt_test')->load($node_6_descendants, FALSE);

		$this
			->assert_instance($node_6_descendants, 'Database_Result')
			->assert_count($node_6_descendants, 5)
			->assert_count($node_6->descendants($query, TRUE)->load($query, FALSE), 6)
	...
	function test_with_descendants()
	{
		$node_6_with_descendants = Sprig::factory('mptt_test', array('id' => 6))->load(NULL, FALSE, array('descendants'));

		$this
			->assert_instance($node_6_with_descendants, 'Database_Result')
			->assert_count($node_6_with_descendants, 5)
			->assert_equal($node_6_with_descendants[0]->descendant->id, 7)
			->assert_equal($node_6_with_descendants[1]->descendant->id, 8)
			->assert_equal($node_6_with_descendants[2]->descendant->id, 9)
			->assert_equal($node_6_with_descendants[3]->descendant->id, 10)
			->assert_equal($node_6_with_descendants[4]->descendant->id, 11);
	}
	...
	function test_siblings()
	{
		...
		$this
			...
			->assert_count($node_5->siblings($query, TRUE)->load($query, FALSE), 4)
			->assert_equal($node_5_siblings[0]->id, 2)
			->assert_equal($node_5_siblings[1]->id, 3)
			->assert_equal($node_5_siblings[2]->id, 6);
	}

	function test_with_siblings()
	{
		$node_5_with_siblings = Sprig::factory('mptt_test', array('id' => 5))->load(NULL, FALSE, array('siblings'));

		$this
			->assert_instance($node_5_with_siblings, 'Database_Result')
			->assert_count($node_5_with_siblings, 3)
			->assert_equal($node_5_with_siblings[0]->sibling->id, 2)
			->assert_equal($node_5_with_siblings[1]->sibling->id, 3)
			->assert_equal($node_5_with_siblings[2]->sibling->id, 6);
	}

	function test_with_siblings_and_parent()
	{
		$node_5_with_siblings = Sprig::factory('mptt_test', array('id' => 5))->load(NULL, FALSE, array('siblings', 'parent'));
	...
	function test_with_leaves()
	{
		$node_1_with_leaves = Sprig::factory('mptt_test', array('id' => 1))->load(NULL, FALSE, array('leaves'));

		$this
			->assert_instance($node_1_with_leaves, 'Database_Result')
			->assert_count($node_1_with_leaves, 2)
			->assert_equal($node_1_with_leaves[0]->leaf->id, 2)
			->assert_equal($node_1_with_leaves[1]->leaf->id, 5);
	}
	...
	function test_insert_as_prev_sibling()
	{
		...
		// Reload node 3 to check insert worked
		$node_3 = Sprig::factory('mptt_test', array('id' => 3))->load();
		$node_3_siblings = $node_3->siblings($query, TRUE)->load($query, FALSE);
	...
	function test_move_to_prev_sibling()
	{
		...
		$node_8_siblings = $node_8->siblings($node_8_siblings, TRUE)->load($node_8_siblings, FALSE);
		$root = $node_8->root();
		$root_children = $root->children();
